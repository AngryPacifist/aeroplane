...
function pillClass(extra = "") {
  return `inline-flex h-10 items-center justify-between gap-3 bg-zinc-800 px-3 text-sm text-zinc-100 ${extra}`.trim();
}
      ...
      <div className="grid gap-4 p-3 md:grid-cols-[1fr_220px]">
        <div className="space-y-3">
          {filters.map((filter, index) => {
            ...
            return (
              <div key={filter.id} className="grid gap-2 md:grid-cols-[44px_128px_minmax(180px,1fr)_minmax(180px,1fr)_minmax(160px,1fr)]">
                <button type="button" className="grid h-10 w-10 place-items-center bg-zinc-800 text-zinc-300 hover:bg-zinc-700 hover:text-white" onClick={() => removeFilter(filter.id)} aria-label="Remove filter">
                ...
                <div className="relative min-w-0">
                  <button type="button" className={`${pillClass("w-full")}`} onClick={() => setOpenMenu(columnMenuOpen ? null : { filterId: filter.id, kind: "column" })}>
                    ...
                    <AppIcon icon={ArrowDown01Icon} size={15} />
                  </button>
                  {columnMenuOpen ? (
                    <div className="absolute left-0 top-full z-50 mt-2 w-[320px] border border-zinc-700 bg-zinc-950 shadow-[0_18px_50px_rgba(0,0,0,0.45)]">
                      <div className="flex items-center gap-2 border-b border-zinc-800 px-3 py-2 text-zinc-400">
                        ...
                        <input
                          ...
                          className="h-9 min-w-0 flex-1 bg-transparent text-sm text-zinc-100 outline-none placeholder:text-zinc-500"
                        />
                      </div>
                      <div className="max-h-72 overflow-y-auto p-2">
                        {filteredColumns.map((column) => (
                          <button
                            key={column.name}
                            type="button"
                            className={`block w-full px-3 py-2 text-left text-sm ${filter.column === column.name ? "bg-zinc-800 text-zinc-50" : "text-zinc-300 hover:bg-zinc-900 hover:text-white"}`}
                ...
                <div className="relative min-w-0">
                  ...
                  {operatorMenuOpen ? (
                    <div className="absolute left-0 top-full z-50 mt-2 w-[260px] border border-zinc-700 bg-zinc-950 p-2 shadow-[0_18px_50px_rgba(0,0,0,0.45)]">
                      {filterOperators.map((item) => (
                        <button
                          key={item.value}
                          type="button"
                          className={`block w-full px-3 py-2 text-left text-sm ${filter.operator === item.value ? "bg-zinc-800 text-zinc-50" : "text-zinc-300 hover:bg-zinc-900 hover:text-white"}`}
                ...
                <input
                  ...
                  className="h-10 min-w-0 bg-zinc-800 px-3 text-sm text-zinc-100 outline-none placeholder:text-zinc-500 disabled:text-zinc-500"
        ...
        <div className="border-zinc-800 md:border-l md:pl-4">
          <button type="button" className="inline-flex h-10 w-full items-center justify-center gap-2 bg-zinc-800 px-3 text-sm text-zinc-100 hover:bg-zinc-700" onClick={addFilter}>
